...
/**
 * a Scenery node that represents a thermometer in the view
 *
 * @author John Blanco
 */
define( function( require ) {
  ...
  // constants
  var TRIANGLE_SIDE_SIZE = 15; // in screen coordinates, which are close to pixels
  var NUM_TICK_MARKS = 13;
  var TICK_MARK_STROKE_LINE_WIDTH = 2;
  ...
  // TODO: Hey, how come there is no thermometer passed in?
  /**
   *
   * @constructor
   */
  function ThermometerNode() {
    ...
    // Add the liquid shaft, the shape of which will indicate the temperature. There are some tweak factors in here for
    // setting the shape of the thermometer liquid.
    ...
    // add the tick marks - there are some tweak factors here that can be adjusted if needed to change appearance
    ...
    for ( var i = 0; i < NUM_TICK_MARKS; i++ ) {
      ...
      var tickMark = new Path( tickMarkShape, {
        stroke: 'black',
        lineWidth: TICK_MARK_STROKE_LINE_WIDTH
      } );
    ...
    // offset is on the left side of the sensing triangle
    ...
    var triangleUpperRightPoint = triangleLeftmostPoint.plus( new Vector2( TRIANGLE_SIDE_SIZE, 0 )
      .rotate( Math.PI / 5 ) );

    var triangleLowerRightPoint = triangleLeftmostPoint.plus( new Vector2( TRIANGLE_SIDE_SIZE, 0 )
      .rotate( -Math.PI / 5 ) );
