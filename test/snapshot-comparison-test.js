...
test( 'snapshot comparison hash check', async ( { page } ) => {
  ...
  const consolePromise = new Promise( ( resolve ) => {
    page.on( 'console', msg => {
      ...
      if ( text.includes( 'Creating td for sim "energy-forms-and-changes"' ) && text.includes( 'hash=' ) ) {
  ...
  await page.goto( 'http://localhost/aqua/html/snapshot-comparison.html?repos=energy-forms-and-changes&logHash&numFrames=20' );
